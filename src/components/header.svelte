    ...
    <div class="flex flex-1 items-center">
      <div class="flex flex-1 flex-col gap-4">
        ...
        <div class="flex items-end justify-end">
          <div class="xs:m-1 xs:w-full flex h-8 cursor-pointer items-center gap-2 rounded-md border-2 border-slate-400 bg-zinc-500/70 bg-gradient-to-r from-fuchsia-500 to-cyan-500 px-2 text-sm font-bold dark:border-slate-500 dark:hover:border-indigo-600 sm:mr-2">
        ...
        <div class="flex justify-end">
          <div class="xs:w-full flex flex-row justify-end rounded-lg bg-zinc-300/90 px-2 py-1 tracking-widest dark:bg-zinc-900/90 sm:mr-2 sm:bg-slate-300/90">
            <Button variant="ghost" on:click={() => ($isMenuOpen = !$isMenuOpen)} class="xs:hidden text-slate-600 sm:visible md:hidden">
              {#if $isMenuOpen}
                <X class="h-6 w-6" />
              {:else}
                <Menu class="h-6 w-6" />
            ...
            {#if $isMenuOpen}
              <div class="xs:flex-col-reverse xs:gap-2 xs:pt-2 flex w-full">
